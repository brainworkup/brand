...
for (concept in concepts) {
  for (fmt_name in names(formats)) {
    ...
    # Brand file path relative from scripts/output/_qmd/
    # Script lives in voice/brand/scripts/
    # Brand files live in voice/brand/concepts/<concept>/_brand.yml
    brand_abs <- normalizePath(
      file.path(dirname(script_dir), "concepts", concept, "_brand.yml"),
      mustWork = TRUE
    )

    brand_rel <- fs::path_rel(brand_abs, start = qmd_dir)

    cli_h2("[{count}/{total}] {concept_labels} -> {fmt_name}")
